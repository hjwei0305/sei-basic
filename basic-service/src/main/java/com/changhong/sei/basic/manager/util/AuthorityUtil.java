...
import com.changhong.sei.basic.manager.*;
...
public class AuthorityUtil {
    ...
    public static void cleanAuthorizedCachesByFeatureRoleId(String featureRoleId) {
        ...
        FeatureRole featureRole = ContextUtil.getBean(FeatureRoleManager.class).findOne(featureRoleId);
        ...
        List<User> users = ContextUtil.getBean(UserFeatureRoleManager.class).getParentsFromChildId(featureRoleId);
        ...
        List<Position> positions = ContextUtil.getBean(PositionFeatureRoleManager.class).getParentsFromChildId(featureRoleId);
    ...
    public static void cleanAuthorizedCachesByDataRoleId(String dataRoleId) {
        ...
        // step 1 检查当前角色是否是公共角色,公共角色清除所有用户
        DataRole dataRole = ContextUtil.getBean(DataRoleManager.class).findOne(dataRoleId);
        ...
        List<User> users = ContextUtil.getBean(UserDataRoleManager.class).getParentsFromChildId(dataRoleId);
        if (CollectionUtils.isNotEmpty(users)) {
            users.forEach(user -> {
                cleanUserAuthorizedCaches(user.getId());
            });
        ...
        List<Position> positions = ContextUtil.getBean(PositionDataRoleManager.class).getParentsFromChildId(dataRoleId);
        if (CollectionUtils.isNotEmpty(positions)) {
            positions.forEach(position -> {
                cleanAuthorizedCachesByPositionId(position.getId());
            });
    ...
    public static void cleanAuthorizedCachesByPositionId(String positionId) {
        ...
        List<Employee> employees = ContextUtil.getBean(EmployeePositionManager.class).getParentsFromChildId(positionId);
