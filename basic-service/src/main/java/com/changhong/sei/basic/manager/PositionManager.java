 ...
 */
@Component
public class PositionManager extends BaseEntityManager<Position> {
    @Autowired
    ...
    protected OperateResult preDelete(String s) {
        if (positionDataRoleManager.isExistByParent(s)) {
            //00039 = 岗位存在已经分配的数据角色，禁止删除！
            return OperateResult.operationFailure("00039");
        }
        if (positionFeatureRoleManager.isExistByParent(s)) {
            //岗位存在已经分配的功能角色，禁止删除！
            return OperateResult.operationFailure("00013");
        }
        if (employeePositionManager.isExistByChild(s)) {
    ...
    public PageResult<Position> findByPositionQueryParam(PositionQueryParam param) {
        ...
        if (param.getIncludeSubNode()) {
            List<Organization> orgs = organizationManager.getChildrenNodes4Unfrozen(param.getOrganizationId());
    ...
    public List<Employee> listAllAssignedEmployeesByPositionId(String positionId) {
        List<Employee> employees = employeePositionManager.getParentsFromChildId(positionId);
    ...
    public List<FeatureRole> getCanAssignedFeatureRoles(String featureRoleGroupId, String positionId) {
        ...
        //获取可分配的功能角色
        List<FeatureRole> canAssigned = featureRoleManager.findByFeatureRoleGroup(featureRoleGroupId);
        //获取已经分配的功能角色
        List<FeatureRole> assigned = positionFeatureRoleManager.getChildrenFromParentId(positionId);
    ...
    public List<DataRole> getCanAssignedDataRoles(String dataRoleGroupId, String positionId) {
        ...
        //获取可分配的数据角色
        List<DataRole> canAssigned = dataRoleManager.findByDataRoleGroup(dataRoleGroupId);
        //获取已经分配的数据角色
        List<DataRole> assigned = positionDataRoleManager.getChildrenFromParentId(positionId);
    ...
    public List<Position> listAllCanAssignPositions(PositionQueryParam param) {
        ...
        List<Position> assignedPositions = employeePositionManager.getChildrenFromParentId(userId);
    ...
    private Position copyPosition(Position position, String orgId, boolean copyFeatureRole){
        ...
        // 复制功能角色
        if (copyFeatureRole){
            List<FeatureRole> roles = positionFeatureRoleManager.getChildrenFromParentId(position.getId());
            if (CollectionUtils.isNotEmpty(roles)){
                List<String> roleIds = roles.stream().map(FeatureRole::getId).collect(Collectors.toList());
                positionFeatureRoleManager.insertRelations(targetPosition.getId(), roleIds);
            }
        }
    ...
    public List<Executor> getExecutorsByPostCatAndOrgToRoot(String orgId, List<String> postCatIds) {
        ...
        if (Objects.nonNull(posts) && !posts.isEmpty()) {
            Organization organization = organizationManager.findOne(orgId);
            //2.循环组织向上查找
    ...
    private List<EmployeePosition> getEmployeesByPostionsAndOrgsToRoot(Organization organization, List<Position> posts) {
        ...
        if (!organization.getFrozen()) {
            ...
            if (!resultPositions.isEmpty()) {
                ...
                employeePositions = employeePositionManager.findByFilter(filter);
            }
        }
        //筛选掉冻结的用户的信息
        ...
        if (Objects.isNull(employeePositions) || employeePositions.isEmpty()) {
            ...
            if (StringUtils.isNotBlank(parentId)) {
                Organization parentOrganization = organizationManager.findOne(organization.getParentId());
